...
MWindowShadow::MWindowShadow(QObject *parent) noexcept
    : QObject(parent)
    , m_view(nullptr)
{
...
MWindowShadow::~MWindowShadow()
{
}

CompositeShadowParams MWindowShadow::lookupShadowParams(int shadowSizeEnum)
{
...
void MWindowShadow::classBegin()
{
    m_shadowTiles = this->shadowTiles();
}

void MWindowShadow::componentComplete()
{
    configureTiles();
}

void MWindowShadow::setView(QWindow *view)
{
    if (view != m_view) {
        m_view = view;
        emit viewChanged();
...
QWindow *MWindowShadow::view() const
{
    return m_view;
}

void MWindowShadow::setGeometry(const QRect &rect)
{
    if (rect != m_rect) {
        m_rect = rect;
...
QRect MWindowShadow::geometry() const
{
    return m_rect;
}

void MWindowShadow::setRadius(qreal value)
{
    if (m_radius != value) {
        m_radius = value;
        emit radiusChanged();
...
qreal MWindowShadow::strength() const
{
    return m_strength;
}

void MWindowShadow::setStrength(qreal strength)
{
...
void MWindowShadow::onViewVisibleChanged(bool visible)
{
    if (visible && m_view) {
        configureTiles();
    }
...
void MWindowShadow::configureTiles()
{
}

TileSet MWindowShadow::shadowTiles()
{
...
QMargins MWindowShadow::shadowMargins(TileSet shadowTiles) const
{
    const CompositeShadowParams params = lookupShadowParams(ShadowVeryLarge);
    if (params.isNone())
        return QMargins();
